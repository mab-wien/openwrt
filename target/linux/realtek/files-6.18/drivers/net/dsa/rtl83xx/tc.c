				    ...
				    struct flow_cls_offload *f, int ingress_port)
{
	...
	if (priv->r->pie_rule_id_is_log_counter) {
		 ...
		 * pie_rule_add() has assigned the ID.
		 *
		 * The implied LOG entry is not reserved in packet_cntr_use_bm,
		 * so it can alias a counter handed out by
		 * rtl83xx_packet_cntr_alloc() for an L3 route PIE rule. That
		 * only matters when hardware L3 offload and tc cls_flower
		 * offload are active on the same switch; a shared LOG-entry
		 * allocator for both paths is left as follow-up work.
		 */
